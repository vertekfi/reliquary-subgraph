...
import { LevelChanged, Reliquary as ReliquaryContract } from '../../generated/Reliquary/Reliquary';
...
export function logPoolAddition(event: LogPoolAddition): void {
  ...
  createPool(params.pid.toI32(), params.allocPoint.toI32(), params.poolToken, params.rewarder, params.nftDescriptor);
}
...
export function logPoolModified(event: LogPoolModified): void {
  ...
  if (params.rewarder == Address.zero()) {
    ...
    const rewarder = getOrCreateRewarder(params.pid.toI32(), Address.fromBytes(params.rewarder));
...
export function setEmissionCurve(event: LogSetEmissionCurve): void {
  const reliquary = getOrCreateReliquary();
  const emissionCurve = getOrCreateEmissionCurve(event.params.emissionCurveAddress);
...
export function deposit(event: Deposit): void {
  ...
  const dailyPoolSnapshot = getOrCreateDailyPoolSnapshot(relic.pid, event.block.timestamp.toI32());
  ...
  dailyPoolSnapshot.dailyDeposited = dailyPoolSnapshot.dailyDeposited.plus(scaledAmount);
...
export function withdraw(event: Withdraw): void {
  ...
  dailyPoolSnapshot.relicCount = pool.relicCount;
  dailyPoolSnapshot.totalBalance = pool.totalBalance;
  dailyPoolSnapshot.dailyWithdrawn = dailyPoolSnapshot.dailyWithdrawn.plus(scaledAmount);
  dailyPoolSnapshot.save();
}
...
export function harvest(event: Harvest): void {
  ...
  if (params.amount.gt(BigInt.zero())) {
    createHarvest(params.relicId.toI32(), params.amount, event.block.timestamp.toI32(), params.to);
  }
}
...
export function transfer(event: Transfer): void {
  if (event.params.from == Address.zero()) {
    ...
    pool.relicCount--;
    pool.save();
    const snapshot = getOrCreateDailyPoolSnapshot(relic.pid, event.block.timestamp.toI32());
